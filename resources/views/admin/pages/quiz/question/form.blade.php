...
<div class="card-body">
    ...
    @foreach($question->imageSizes as $image)
    <div class="mb-3">
        <div class="form-label">{{ trans('admin.page.quiz.questions.form.'.$image) }}</div>
        @if(is_null($question->getPublicMediaUrl($image)))
        ...
        @else
            <div class="form-group">
                ...
                <a href="{{route('admin.quizzes.questions.remove_image', ['quiz' => $quiz->id, 'question' => $question->id, 'size' => $image])}}" class="btn btn-danger btn-xs" style="width:25px; margin:0; padding-left:25px; height: 25px" title="{{ trans('admin.button.remove_image') }}">
            ...
            <img src="{{$question->getPublicMediaUrl($image)}}" alt="" height="50" />
        @endif
    </div>
    @endforeach
</div>
